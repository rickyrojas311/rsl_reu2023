...
def display_DMI_res():
    ...
    """
    #Ground Truths
    glx_header = nib.as_closest_canonical(nib.load(r"project_data/BraTS_Data/DMI_Simulations/DMI/patient_9/DMI_patient_9_ds_2_gm_3.0_wm_1.0_tumor_0.5_ed_2.0_noise_0_seed_1234/dmi_gt.nii.gz"))
    ...
    #Structural Data
    structural_header = nib.as_closest_canonical(nib.load(r"project_data/BraTS_Data/DMI_Simulations/DMI/patient_9/t1.nii.gz"))
    structural_data = structural_header.get_fdata()
    structural_data = xp.array(normalize_matrix(structural_data))

    #Low Res Data
    mm4_header = nib.as_closest_canonical(nib.load(r"project_data/BraTS_Data/DMI_Simulations/DMI/patient_9/DMI_patient_9_ds_2_gm_3.0_wm_1.0_tumor_0.5_ed_2.0_noise_0_seed_1234/dmi.nii.gz"))
    data_4mm = mm4_header.get_fdata()[:, :, :, 0][::2, ::2, ::2]
    data_4mm = xp.array(normalize_matrix(data_4mm))
    mm12_header = nib.as_closest_canonical(nib.load(r"project_data/BraTS_Data/DMI_Simulations/DMI/patient_9/DMI_patient_9_ds_6_gm_3.0_wm_1.0_tumor_0.5_ed_2.0_noise_0_seed_1234/dmi.nii.gz"))
    data_12mm = mm12_header.get_fdata()[:, :, :, 0][::6, ::6, ::6]
    data_12mm = xp.array(normalize_matrix(data_12mm))
    mm24_header = nib.as_closest_canonical(nib.load(r"project_data/BraTS_Data/DMI_Simulations/DMI/patient_9/DMI_patient_9_ds_12_gm_3.0_wm_1.0_tumor_0.5_ed_2.0_noise_0_seed_1234/dmi.nii.gz"))
    ...
    ax[0].set_xlabel("Glx Orig")
    ax[0].set_title("GT/Struct")
    ax[0].axis("off")
    ax[1].imshow(data_4mm[:, :, SLICE//2].get(), vmin = 0, vmax = glx_gt.max(), cmap = 'Greys_r')
    ax[1].set_title("4mm")
    ax[1].axis("off")
    ax[2].imshow(data_12mm[:, :, SLICE//6].get(), vmin = 0, vmax = glx_gt.max(), cmap = 'Greys_r')
    ax[2].set_title("12mm")
    ax[2].axis("off")
    ax[3].imshow(data_24mm[:, :, SLICE//12].get(), vmin = 0, vmax = glx_gt.max(), cmap = 'Greys_r')
    ax[3].set_title("24mm")
    ax[3].axis("off")
